...
protected:
	...
	void onConnectComplete(SOCKET socket, DWORD dwTransferred, OV_DATA * pov, int error)
	{
		...
		if (pd->proxyType == PROXY_HTTPS)
		{
			...
			pAddr = (sockaddr*)&pd->connInfo.remoteAddress;
			...
			if (startTcpSend(pd, OUT_SOCKET, (char*)request, (int)strlen(request), pov->id))
			{
	...
	void onAcceptComplete(SOCKET socket, DWORD dwTransferred, OV_DATA * pov, int error)
	{
...

		{
			...
			int result = WSAIoctl(acceptSocket,
				  ...
				  0, NULL,
				  pd->connInfo.remoteAddress, NF_MAX_ADDRESS_LENGTH,
				  &retLen, NULL, NULL);
			...
			if (result == 0)
			{
				...
				if (pd->proxyType == PROXY_HTTPS)
				{
					...
					DbgPrint("TCPProxy::onAcceptComplete() redirect to HTTPS proxy %s", remoteAddr);
#endif
