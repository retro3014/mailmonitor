...
// Naver Mail server IPs (resolved at startup for port 443 filtering)
static std::vector<std::string> g_naverMailIps;
static CRITICAL_SECTION g_naverIpLock;

// Resolve mail.naver.com IPs at startup
static void resolveNaverMailIps()
{
    InitializeCriticalSection(&g_naverIpLock);

    const char* hosts[] = { "mail.naver.com", NULL };
    ...
    for (int h = 0; hosts[h] != NULL; h++)
    {
        ...
        if (getaddrinfo(hosts[h], NULL, &hints, &result) == 0)
        {
            for (struct addrinfo* p = result; p != NULL; p = p->ai_next)
            {
                ...
                std::string ip(ipBuf);
                bool exists = false;
                for (size_t i = 0; i < g_naverMailIps.size(); i++)
                {
                    if (g_naverMailIps[i] == ip) { exists = true; break; }
                }
                if (!exists)
                {
                    g_naverMailIps.push_back(ip);
                    printf("[Config] Naver Mail IP: %s (from %s)\n", ipBuf, hosts[h]);
                }
            }
            freeaddrinfo(result);
        }
    ...
    if (g_naverMailIps.empty())
    {
        printf("[WARNING] Could not resolve any Naver Mail IPs.\n");
        printf("          HTTPS Naver Mail interception will not work.\n");
        g_httpsProxyEnabled = false;
    }
    else
    {
        printf("[Config] Resolved %d Naver Mail IP(s) for HTTPS interception\n",
            (int)g_naverMailIps.size());
    }
}

// Check if an IP belongs to Naver Mail servers
static bool isNaverMailIp(const std::string& ip)
{
    // Exact match against resolved IPs first
    for (size_t i = 0; i < g_naverMailIps.size(); i++)
    {
        if (g_naverMailIps[i] == ip) return true;
    }

    // Naver's well-known IP prefixes. mail.naver.com currently resolves
    // into 125.209.x.x / 223.130.x.x ranges; we also match common Naver
    // CDN ranges to be safe in case DNS rotates onto a sibling subnet.
    if (ip.find("125.209.") == 0) return true;
    if (ip.find("223.130.") == 0) return true;
    if (ip.find("210.89.") == 0)  return true;
    if (ip.find("117.52.") == 0)  return true;
    if (ip.find("121.78.") == 0)  return true;
...
class SmtpEventHandler : public NF_EventHandler
{
    ...
    virtual void tcpConnectRequest(ENDPOINT_ID id, PNF_TCP_CONN_INFO pConnInfo)
    {
        ...
        // HTTPS port 443: intercept only Naver Mail server connections
        if (remotePort == 443 && g_httpsProxyEnabled && g_tlsProxyEnabled &&
            g_certGen.isInitialized() && isNaverMailIp(remoteIp))
        {
            // Quiet: only log when mail.naver.com MITM actually starts
...
void printUsage()
{
    ...
    printf("  -no_https           Disable HTTPS interception (Naver Mail web)\n");
    ...
    printf("  6) HTTPS interception for Naver Mail web (mail.naver.com)\n\n");
    ...
    printf("  4) For Naver Mail web: disable QUIC in Chrome (chrome://flags)\n");
}
...
int main(int argc, char* argv[])
{
    ...
    // Resolve Naver Mail server IPs for HTTPS interception
    ...
    // Rule 4: HTTPS port 443 (for Naver Mail web interception)
    ...
    if (g_tlsProxyEnabled)
    {
        ...
        if (g_httpsProxyEnabled)
        {
            printf("[OK] HTTPS interception active: port 443 (Naver Mail) -> local proxy %d\n", g_proxyPort);
            printf("     Naver Mail web (mail.naver.com) will be monitored.\n");
